...
<launch>
  ...
  <node name="conveyor_spawner" pkg="casus_gazebo" type="conveyor_spawner_node" output="screen">
    <rosparam command="load" file="$(find casus_gazebo)/config/conveyor_object.yaml"/>
  </node>

  <node name="spawn_object_once" pkg="casus_utilities" type="spawn_object_once.py" output="screen"/>
  <node name="breakbeam_conveyor_stop" pkg="casus_utilities" type="breakbeam_conveyor_stop.py" output="screen"/>
